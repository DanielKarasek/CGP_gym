...
class Genome:
    ...
    def distance(self, other: "Genome") -> int:
        ...
        """
        self._refresh_active_nodes_list()
        other._refresh_active_nodes_list()
        shorter_active_nodes = (self.active_nodes if len(self.active_nodes) < len(other.active_nodes)
                                else other.active_nodes)
        longer_active_nodes = (self.active_nodes if len(self.active_nodes) >= len(other.active_nodes)
                               else other.active_nodes)
        pointer_longer = 0
        inverse_distance = 0
        for pointer_shorter in np.arange(len(shorter_active_nodes)):
            while (pointer_longer < len(shorter_active_nodes) and
                   shorter_active_nodes[pointer_shorter].idx > longer_active_nodes[pointer_longer].idx):
                pointer_longer += 1
            if pointer_longer == len(longer_active_nodes):
                break
            node_1 = shorter_active_nodes[pointer_shorter]
            node_2 = longer_active_nodes[pointer_longer]
            inverse_distance += node_1.inv_distance(node_2)
                
        return (len(longer_active_nodes)-inverse_distance)/(len(longer_active_nodes)+1e-10)
    #xkaras38
    def _refresh_active_nodes_list(self):
        """
        If active nodes list is not valid(wasn't calculated before, or mutation occured),
         it calculates it and stores it in self.active_nodes
        :return:
        """
        if self.is_active_nodes_valid:
            return
        ...
        self.active_nodes = [item for subset in dict_of_active.values() for item in subset]
        self.active_nodes = sorted(self.active_nodes, key=lambda node: node.idx)
        self.is_active_nodes_valid = True
