...
class Node:
    ...
    #xkaras38
    def inv_distance(self, other):
        inv_dist = 0.5 * (self.__class__.__name__ == other.__class__.__name__ and
                      self.idx == other.idx)
        if inv_dist == 0:
            return 0

        inv_dist += len([1 for a, b in zip(self.addresses, other.addresses) if a == b])/self.arity/2
        return inv_dist
